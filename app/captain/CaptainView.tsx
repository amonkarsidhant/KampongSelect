...
function roleIcon(role: string) {
  switch (role) {
    ...
    case "Bowler":
      return <Activity size={14} className="text-crimson-400" />;
    case "All-rounder":
      return (
        <div className="flex -space-x-1">
          <Star size={12} className="text-amber-400" />
          <Activity size={12} className="text-crimson-400" />
        </div>
      );
    case "Wicketkeeper":
      return <Hand size={14} className="text-emerald-400" />;
    default:
      return <Star size={14} />;
  }
}
      ...
      <div className="grid grid-cols-12 gap-3 items-center px-4 py-3">
        ...
        <div className="col-span-5 relative">
          <div className="flex items-center gap-2">
            <p
              className={cn(
                "text-sm font-medium",
                isTaken && "line-through text-foreground-muted",
              )}
            >
              {player.full_name}
            </p>
            ...
            {isExcused && (
              <span className="text-[8px] bg-blue-500/20 text-blue-300 px-1 rounded font-bold uppercase">
                Excused
              </span>
            )}
          </div>
        ...
        <div className="col-span-1 flex justify-end">
          <div
            ...
            title={
              response
                ? `Responded: ${response.toUpperCase()}`
                : "No response yet"
            }
          />
        </div>
...
export default function CaptainView({
  ...
  const pool = useMemo(() => {
    ...
    const availForDate = new Map<string, AvailStatus>(
      availability
        .filter((a) => a.match_date === match.match_date)
        .map((a) => [a.player_id, a.status]),
    );
    const availDetails = availability.filter(
      (a) => a.match_date === match.match_date,
    );
  ...
  const higherTeams = teams.filter(
    (t) =>
      t.tier_order != null && tierOrder != null && t.tier_order < tierOrder,
  );
  ...
  const filteredPool = useMemo(() => {
    return pool.filter((p) => {
      if (
        ...
        return false;
      if (roleFilter && p.player.role !== roleFilter) return false;
      if (tierFilter && p.player.tier?.toString() !== tierFilter) return false;
      if (statusFilter) {
        if (statusFilter === "yes" && p.response !== "yes") return false;
        if (statusFilter === "no" && p.response !== "no") return false;
        if (statusFilter === "pending" && p.response !== null) return false;
        if (statusFilter === "excused" && !p.is_excused) return false;
        if (statusFilter === "taken" && p.status !== "taken_by_higher")
          return false;
      }
      return true;
    });
  }, [pool, searchQuery, roleFilter, tierFilter, statusFilter]);
  ...
  function togglePick(playerId: string) {
    ...
    if (existing) {
      ...
      startTransition(async () => {
        ...
        if (data) {
          setSelections((prev) =>
            prev.map((s) => (s.id === optimistic.id ? (data as Selection) : s)),
          );
        }
      });
    }
  ...
  function toggleRole(playerId: string, role: "is_captain" | "is_keeper") {
    const existing = selections.find(
      (s) => s.match_id === matchId && s.player_id === playerId,
    );
    if (!existing) return;
    const nextValue = !existing[role];
    setSelections((prev) =>
      prev.map((s) => (s.id === existing.id ? { ...s, [role]: nextValue } : s)),
    );
    startTransition(async () => {
      await supabase
        .from("selections")
        .update({ [role]: nextValue })
        .eq("id", existing.id);
    });
  }
  ...
  async function confirmXI() {
    ...
    const warnings = [];
    const hasCaptain = myPicks.some((s) => s.is_captain);
    const hasKeeper = myPicks.some((s) => s.is_keeper);
    if (!hasCaptain) warnings.push("No captain selected.");
    if (!hasKeeper) warnings.push("No wicketkeeper selected.");

    const questionablePicks = myPicks.filter((s) => {
      const p = pool.find((x) => x.player.id === s.player_id);
      return p && p.response !== "yes";
    });

    if (questionablePicks.length > 0) {
      warnings.push(
        `You have selected ${questionablePicks.length} player(s) who haven't explicitly said 'Yes'.`,
      );
    }
    ...
    startTransition(async () => {
      ...
      if (!error) {
        ...
        alert(
          `XI confirmed for ${match.team_code} vs ${match.opposition}. The Pulse is notifying your players.`,
        );
      }
    });
  }

  async function reopenXI() {
    if (!match) return;
    const ok = confirm(
      "Are you sure you want to reopen this XI? This will move it back to 'Selecting' state.",
    );
    if (!ok) return;

    startTransition(async () => {
      await supabase.from("match_status").upsert({
        match_id: match.id,
        state: "selecting",
        confirmed_at: null,
        confirmed_by: null,
  ...
  function copyWhatsAppBriefing() {
    if (!match) return;
    const dateStr = new Date(match.match_date).toLocaleDateString("en-GB", {
      weekday: "long",
      day: "numeric",
      month: "long",
    });
    const squadNames = myPicks
      .map((s, i) => {
        const p = players.find((x) => x.id === s.player_id);
...
https://kampong-select.vercel.app`;
  ...
  const visibleTeams =
    me.user_role === "admin"
      ? teams
      : teams.filter((t) => t.code === myTeamCode);
  ...
  return (
    <main className="min-h-screen pb-20">
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div
          className={cn(
            "flex items-end justify-between mb-2 pb-4 border-b transition-colors duration-1000",
            availableCount < 11 ? "border-warning/50" : "border-border",
          )}
        >
          <div>
            <h1
              className={cn(
                "font-display text-4xl font-semibold tracking-tight transition-colors duration-1000",
                availableCount < 11 ? "text-warning" : "text-white",
              )}
            >
              Selection
            </h1>
            <p className="text-foreground-muted font-mono text-sm mt-1">
              {me.captains_team ?? "Admin"}
            </p>
          </div>
          <div
            ...
            <motion.div
              ...
              className={cn(
                ...
                xiCount === 11
                  ? "text-success"
                  : availableCount < 11
                    ? "text-warning animate-pulse-slow"
                    : "text-white",
              )}
            >
              {xiCount}
            </motion.div>
            <span className="font-display text-5xl font-black text-border mx-1">
              /
            </span>
            <span className="font-display text-5xl font-bold text-foreground-muted">
              11
            </span>
          </div>
        </div>
          ...
          {Object.entries(squadBalance).map(([role, count]) => (
            <div key={role} className="flex items-center gap-2 flex-shrink-0">
              <div className="bg-white/5 border border-border rounded-lg px-3 py-1.5 flex items-center gap-2">
                {roleIcon(role)}
                <span className="text-[10px] font-mono font-bold text-foreground-muted uppercase tracking-wider">
                  {role}s
                </span>
                <span
                  className={cn(
          ...
          <div className="overflow-hidden bg-warning/10 border-y border-warning/20 mb-6 py-2">
            ...
            >
              <p className="text-xs font-mono font-bold text-warning uppercase tracking-widest inline-block">
                CASCADE UPDATE • {takenCount} PLAYERS SECURED BY HIGHER TIERS (
                {higherTeams.map((t) => t.code).join(", ")}) • REMOVED FROM
                CURRENT POOL
              </p>
              <p className="text-xs font-mono font-bold text-warning uppercase tracking-widest inline-block">
                CASCADE UPDATE • {takenCount} PLAYERS SECURED BY HIGHER TIERS (
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex gap-2 flex-1 overflow-x-auto pb-2 md:pb-0">
            {me.user_role === "admin" && (
              <select
                value={selectedTeam}
            ...
            {teamMatches.length > 0 && (
              <select
                ...
                className="bg-surface border border-border rounded-lg px-3 py-2 text-sm font-bold focus:outline-none min-w-[160px]"
              >
                {teamMatches.map((m) => (
            ...
            <input
              ...
              className="bg-surface border border-border rounded-lg px-3 py-2 text-sm focus:outline-none min-w-[120px]"
            />
            <select
              value={roleFilter}
            ...
            >
              ...
              <option value="taken">Taken by Higher</option>
            </select>
          </div>
          <button
            onClick={copyWhatsAppBriefing}
            className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all active:scale-95 whitespace-nowrap"
          >
          ...
          <div className="bg-surface border border-border rounded-2xl overflow-hidden shadow-2xl">
                ...
                filteredPool.map(
                  ({
                    ...
                    const sel = selections.find(
                      (s) =>
                        s.match_id === matchId && s.player_id === player.id,
                    );
                    return (
                      <PlayerRow
                        key={player.id}
            ...
            <div className="p-4 bg-background/50 border-t border-border flex justify-between items-center">
              <p className="font-mono text-sm text-foreground-muted">
                {xiCount === 11 ? (
                  <span className="text-success font-bold">11/11 SELECTED</span>
                ) : (
                  `${xiCount}/11 SELECTED`
                )}
              </p>
              <div className="flex gap-2">
                {me.user_role === "admin" &&
                  matchStatus.find((s) => s.match_id === match.id)?.state ===
                    "confirmed" && (
                    <button
                      onClick={reopenXI}
                      className="px-4 py-2.5 rounded-lg font-bold transition-all bg-surface text-foreground-muted border border-border hover:bg-white/5"
                    >
                      Reopen XI
                    </button>
                  )}
